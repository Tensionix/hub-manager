...
SUPPORTED_STATUS_MODES = {"labeled", "icons", "letters"}
...
class UiSettings:
    ...
    header_status_mode: str = "labeled"
...
def _safe_status_mode(value: Any) -> str:
    text = str(value or "labeled").strip().lower()
    return text if text in SUPPORTED_STATUS_MODES else "labeled"
...
def load_ui_settings(path: Path) -> UiSettings:
    ...
    return UiSettings(
        ...
        header_status_mode=_safe_status_mode(ui_data.get("header_status_mode", "labeled")),
    )
...
def save_ui_settings(path: Path, settings: UiSettings) -> None:
    ...
    text = (
        ...
        f"  header_status_mode: \"{_safe_status_mode(settings.header_status_mode)}\"\n"
    )
