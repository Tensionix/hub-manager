...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Audion Hub Manager GUI window.")
...
def start_server(host: str, port: int) -> subprocess.Popen[str]:
    ...
    env["AHM_OPEN_BROWSER"] = "0"
    server = subprocess.Popen(
        [sys.executable, str(APP_PY), "--host", host, "--port", str(port), "--no-browser"],
        cwd=str(ROOT),
        stdout=stdout,
        stderr=stderr,
        text=True,
...
def main() -> int:
    ...
    try:
        webview.create_window("Audion Hub Manager", url, width=1700, height=900, min_size=(1180, 720))
        webview.start(
            gui=args.gui,
            debug=False,
            private_mode=False,
