...
function Convert-ToAppId {
    ...
    # A name that already carries the brand keeps it: prefixing "Audion.Tools."
    # onto "Audion Hub Manager" produced Audion.Tools.Audion.Hub.Manager, which
    # is what Windows groups taskbar windows by.
    if ($slug -match '^Audion($|\.)') {
        return $slug
    }
