...
title Audion Hub Manager - Make Release Archive
...
:BANNER
echo ======================================================================
echo   AUDION HUB MANAGER - MAKE RELEASE ARCHIVE
echo ======================================================================
echo Root:    %ROOT%
...
call :REMOVE_STAGE_FILES "*.local.json"
call :REMOVE_STAGE_FILES "*.local.yaml"
call :REMOVE_STAGE_FILES "*.local.yml"
call :REMOVE_STAGE_FILES "*.local.toml"
call :REMOVE_STAGE_ENV_FILES
call :REMOVE_STAGE_FILES "*.key"
call :REMOVE_STAGE_FILES "*.pem"
call :REMOVE_STAGE_FILES "*.pfx"
call :REMOVE_STAGE_FILES "*.p12"
goto :eof
...
:REMOVE_STAGE_FILES
for /f "delims=" %%F in ('dir /a-d /b /s "%STAGE_PROJECT%\%~1" 2^>nul') do del /f /q "%%~fF" >nul 2>nul
goto :eof

:REMOVE_STAGE_ENV_FILES
for /f "delims=" %%F in ('dir /a-d /b /s "%STAGE_PROJECT%\.env*" 2^>nul') do (
  if /I not "%%~nxF"==".env.example" if /I not "%%~nxF"==".env.sample" if /I not "%%~nxF"==".env.template" del /f /q "%%~fF" >nul 2>nul
)
goto :eof
