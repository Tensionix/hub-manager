...
title Audion Hub Manager - Cleanup
...
call :CheckMarker "%BASE_DIR%\config\projects.json" || goto abort
call :CheckMarker "%BASE_DIR%\config\projection_profiles.json" || goto abort

echo ======================================================================
echo   AUDION HUB MANAGER - LEAN SOURCE CLEANUP
echo ======================================================================
echo Root:
echo   %BASE_DIR%
echo.
echo This script returns Audion Hub Manager to a small source-only / LEAN state.
echo Target shape: Git-tracked project structure plus preserved PDF artifacts.
...
echo   - input, output, logs, report, workspace, data, backup, release contents except PDF
...
echo   - generated/cache files, archives, binaries, local overrides and secrets
echo.
echo PDF files are kept for pre-release documentation/PDF-generation workflows.
echo.
...
call :RemoveNamedDirs "._runtime"
call :RemoveNamedDirs "_pwsh_tmp"
call :RemoveNamedDirs "_powershell_tmp"
call :RemoveNamedDirs "_fzf_tmp"
...
call :RemoveFilesByPattern "*.old"
call :RemoveFilesByPattern "*.orig"
call :RemoveFilesByPattern "*.backup"
call :RemoveFilesByPattern "*.swp"
call :RemoveFilesByPattern "*.swo"
call :RemoveFilesByPattern "*.rej"
call :RemoveFilesByPattern "*.cache"
call :RemoveFilesByPattern "*.pid"
call :RemoveFilesByPattern "*.coverage"
call :RemoveFilesByPattern ".coverage"
call :RemoveFilesByPattern "*.local.json"
call :RemoveFilesByPattern "*.local.yaml"
call :RemoveFilesByPattern "*.local.yml"
call :RemoveFilesByPattern "*.local.toml"
call :RemoveEnvFiles
call :RemoveFilesByPattern "*.key"
call :RemoveFilesByPattern "*.pem"
call :RemoveFilesByPattern "*.pfx"
call :RemoveFilesByPattern "*.p12"
call :RemoveFilesByPattern "*.zip"
call :RemoveFilesByPattern "*.7z"
call :RemoveFilesByPattern "*.rar"
call :RemoveFilesByPattern "*.iso"
call :RemoveFilesByPattern "*.img"
call :RemoveFilesByPattern "*.tar"
call :RemoveFilesByPattern "*.gz"
call :RemoveFilesByPattern "*.tgz"
call :RemoveFilesByPattern "*.exe"
call :RemoveFilesByPattern "*.dll"
call :RemoveFilesByPattern "*.pyd"
call :RemoveFilesByPattern "*.msi"
call :RemoveFilesByPattern "*.whl"
call :RemoveFilesByPattern "*.so"
call :RemoveFilesByPattern "*.dylib"
call :RemoveFilesByPattern "*.o"
call :RemoveFilesByPattern "*.obj"
call :RemoveFilesByPattern "*.lib"
call :RemoveFilesByPattern "*.a"
call :RemoveFilesByPattern "*.class"
call :RemoveFilesByPattern "*.jar"
call :RemoveFilesByPattern "*.war"
call :RemoveFilesByPattern "*.wasm"
call :RemoveFilesByPattern "*.map"
call :RemoveFilesByPattern "*.min.js"
call :RemoveFilesByPattern "*.min.css"
call :RemoveFilesByPattern "*.mp4"
call :RemoveFilesByPattern "*.mov"
call :RemoveFilesByPattern "*.mkv"
call :RemoveFilesByPattern "*.avi"
call :RemoveFilesByPattern "*.wav"
call :RemoveFilesByPattern "*.flac"
...
echo [OK] Audion Hub Manager is back to source-only LEAN shape.
echo [OK] runtime, wheelhouse, install\download and generated work folders were recreated empty.
echo [OK] PDF documentation artifacts were preserved.
goto cleanup_done
...
for /f "delims=" %%D in ('dir /ad /b /s "%BASE_DIR%" 2^>nul') do (
  if /I "%%~nxD"=="%DIR_NAME%" (
    if "%DRY_RUN%"=="1" (
      dir /a-d /b /s "%%~fD\*.pdf" >nul 2>nul
      if errorlevel 1 (
        echo   [DRY] rmdir /s /q "%%~fD"
      ) else (
        echo   [DRY] clear non-PDF files under "%%~fD"
      )
    ) else (
      dir /a-d /b /s "%%~fD\*.pdf" >nul 2>nul
      if errorlevel 1 (
        ...
        echo   clear non-PDF files under "%%~fD"
        call :ClearDir "%%~fD"
      )
    )
  )
...
for /f "delims=" %%F in ('dir /a-d /b /s "%TARGET_DIR%" 2^>nul') do (
  if /I "%%~xF"==".pdf" (
    echo   keep PDF "%%~fF"
  ) else (
    if "%DRY_RUN%"=="1" (
...
for /f "delims=" %%D in ('dir /ad /b /s "%TARGET_DIR%" 2^>nul ^| sort /R') do (
  if "%DRY_RUN%"=="1" (
    echo   [DRY] rmdir if empty "%%~fD"
  ) else (
    rd "%%~fD" >nul 2>nul
  )
)
...
if "%DRY_RUN%"=="1" (
  dir /a-d /b /s "%TARGET%\*.pdf" >nul 2>nul
  if not errorlevel 1 (
    echo   [DRY] clear non-PDF files under "%TARGET%"
    call :ClearDir "%TARGET%"
  ) else (
...
dir /a-d /b /s "%TARGET%\*.pdf" >nul 2>nul
if not errorlevel 1 (
  echo   clear non-PDF files under "%TARGET%"
  call :ClearDir "%TARGET%"
  exit /b 0
)
echo   rmdir "%TARGET%"
...
:RemoveEnvFiles
echo [FILES] Removing .env and .env.* secrets
for /f "delims=" %%F in ('dir /a-d /b /s "%BASE_DIR%\.env*" 2^>nul') do (
  if /I not "%%~nxF"==".env.example" if /I not "%%~nxF"==".env.sample" if /I not "%%~nxF"==".env.template" (
    if "%DRY_RUN%"=="1" (
      echo   [DRY] del "%%~fF"
    ) else (
      echo   del "%%~fF"
